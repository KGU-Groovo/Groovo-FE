...
assert.equal(readFileSync('app/live-feedback/index.tsx', 'utf8').includes("pathname: '/result'"), true, 'feedback end must open the result screen');
...
assert.equal(readFileSync('hooks/use-ai-feedback-socket.ts', 'utf8').includes('keypoints,'), true, 'AI feedback hook must send realtime frames');
